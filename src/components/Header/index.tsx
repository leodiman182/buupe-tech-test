...
const MobileFilterSection = ({ handleToggleDrawer }: { handleToggleDrawer: () => void }) => {
  return (
    <div
      className="w-full lg:hidden bg-gradient-to-r from-secondary to-primary py-2 px-6 flex flex-row items-center justify-between">
...
export default function Header() {
  const { searchQuery, setSearchQuery, isFilterDrawerOpen, setIsFilterDrawerOpen, priceFilter } =
    useAppContext();
  ...
  return (
    ...
    >
      <section
        ...
        <form onSubmit={ handleSearch } className="w-full lg:max-w-xl flex flex-row items-center gap-2">
          <SearchInput query={ query } handleChange={ handleChange } handleXClick={ handleXClick }/>
          <Button data-testid={ searchButtonComponent }>Buscar</Button>
        </form>
        <div className="hidden lg:block"/>
      </section>
      <MobileFilterSection handleToggleDrawer={ handleToggleDrawer }/>
    </header>
  );
}